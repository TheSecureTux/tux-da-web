...
groupSubdomains(){
  cat $dir/asset_subs.txt $dir/subfinder.txt  | sort -u > $dir/subdomains.txt
...
scanGau(){
  ...
  gau $domain > $dir/paramlist/gau.txt
...
corsDetect(){
  #corsy
  corsy  -i $dir/live_subdomains.txt -o $dir/corsy.json
}
fuzzer(){
 echo -e "\e[91m------------------- ffuf fuzzing subs ------------------------------------------------\e[0m" 
  ffuf -u https://FUZZ.$domain  -w /usr/share/wordlists/discovery/subdomains-top1million-110000.txt | tee $dir/fuzzsubs.txt

}

findSecrets(){
   for i in $(cat subdomains.txt ) ; do python ~/tools/secretfinder/SecretFinder.py -e -o cli -i https://$i ;done > $dir/paramlist/secrets.txt
